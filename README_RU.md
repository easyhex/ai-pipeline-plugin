...
Плагин для Claude Code — пайплайн AI-разработки из 9 команд с обкаткой пользователя frontier-раундами и явным гейтом подписи спеки, автоматическим критиком (senior engineer) на двух стадиях, системой накопления уроков, памятью Serena для стабильных знаний о проекте и гейтом визуальной верификации через Playwright MCP — всего 6 слоёв контекста.
...
Вы никогда не вводите `/brainstorm`, `/plan`, `/build`, `/critic`, `/verify` или `/finish` — это внутренние фазы девяти команд выше.
...
Всё. 9 команд и агент `senior-critic` теперь доступны глобально в любом проекте.
...
1. **9 пользовательских команд** (в `commands/`) — загружаются Claude Code автоматически
...
- `CLAUDE.md` — правила пайплайна и набор из 9 команд
