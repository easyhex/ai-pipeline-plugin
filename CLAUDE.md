...
- v1.1.0: /init adopt mode for existing codebases (derive Master Plan from real code + git history, never overwrite, commit only pipeline files) + fixes the inverted Master Plan pre-flight (a missing architecture.md now reads as unfilled). Minor per semver: adopt is new functionality.
