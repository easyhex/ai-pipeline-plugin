...
1. **Cursor:** read `docs-meta/.lesson-cursor` (last processed lesson filename; missing → process all). Collect lessons newer than the cursor plus a 3-file overlap window (reruns catch missed clusters).
2. **Cluster** by trigger/root-cause similarity. A cluster of ≥2 lessons (or one lesson whose prevention generalizes) becomes ONE rule.
