...
- Skip the init
- Commit only the pipeline files (never `-A` in a pre-existing repo): `git add CLAUDE.md docs/ docs-meta/ .claude/ .gitignore && git commit -m "chore: pipeline scaffold via /init"`
