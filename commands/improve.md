...
- **Phase 9 (Verify, including 9b visual and 9c quant):** Inherited from `/feature` Phase 9 — visual sub-step runs for frontend projects; the 9c quant gate runs against the TARGET feature's requirements file (its NFR proving commands — including any amended at Phase 3.5), same mode resolution. The brainstorm in Phase 2 must include `## URLs to verify` listing the affected paths for frontend work. For an improvement that doesn't change any URL, list at least `/`.
